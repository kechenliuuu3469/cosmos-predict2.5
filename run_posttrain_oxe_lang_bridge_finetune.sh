#!/bin/bash
#SBATCH --job-name=cosmos_oxe_lang_bridge_ft
#SBATCH --nodes=1
...
mkdir -p slurm_outputs/cosmos_oxe_lang_bridge_ft
...
# UPDATE THIS: set to your Stage 1 OXE language checkpoint path
export OXE_LANG_STAGE1_CKPT_PATH=/path/to/oxe_lang_stage1_checkpoint

PYTHONPATH=$(pwd) \
torchrun --nproc_per_node=4 --master_port=12351 \
    -m scripts.train \

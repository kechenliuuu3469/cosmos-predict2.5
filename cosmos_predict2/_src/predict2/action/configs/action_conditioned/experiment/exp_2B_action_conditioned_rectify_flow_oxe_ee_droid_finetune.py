"""
Stage 2: Fine-tune on DROID (DreamZero stacking) after OXE EE pre-training.

Loads from Stage 1 OXE EE checkpoint (same action_dim=7).
All weights transfer perfectly (strict_resume=True).
Fresh optimizer state for fine-tuning.
Uses DROID DreamZero-style multi-view stacking.
"""
...
ac_reason_embeddings_rectified_flow_2b_256_320_oxe_ee_droid_finetune = LazyDict(
    dict(
        defaults=[
        ...
        checkpoint=dict(
            save_iter=5_000,
            # UPDATE THIS to point to your Stage 1 OXE EE checkpoint
            load_path="${oc.env:OXE_EE_STAGE1_CKPT_PATH,/path/to/oxe_ee_stage1_checkpoint}",
...
for _item in [ac_reason_embeddings_rectified_flow_2b_256_320_oxe_ee_droid_finetune]:

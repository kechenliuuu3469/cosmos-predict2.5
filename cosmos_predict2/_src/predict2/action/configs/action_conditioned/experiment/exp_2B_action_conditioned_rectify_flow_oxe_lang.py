"""
Stage 1: OXE language pre-training.

Pre-train Cosmos on combined OXE datasets with language descriptions
of motion as the conditioning signal. Real numerical actions are set
to zero -- the model conditions only on language via compute_online=True.

After pre-training, fine-tune on Bridge or DROID with real actions (Stage 2).
"""
...
ac_reason_embeddings_rectified_flow_2b_256_320_oxe_lang = LazyDict(
    dict(
        defaults=[
            ...
            {"override /data_train": "oxe_lang_train"},
            {"override /data_val": "oxe_lang_val"},
            "_self_",
        ],
        job=dict(
            project="cosmos_predict2_action_conditioned",
            group="cosmos_predict_v2p5",
            name="2b_oxe_lang_action_conditioned",
        ),
        optimizer=dict(
            lr=32e-5,
            weight_decay=0.1,
        ...
        checkpoint=dict(
            save_iter=10_000,
            load_path="308eb96c-c4c0-4a06-9cc1-103a43beff28",  # Base Cosmos 2B pre-trained
        ...
        model=dict(
            config=dict(
                ...
                state_t=1 + 12 // 4,
                net=dict(
                    action_dim=7,        # Keep MLP, but actions will be zeros
...
for _item in [ac_reason_embeddings_rectified_flow_2b_256_320_oxe_lang]:

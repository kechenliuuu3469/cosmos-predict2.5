#!/bin/bash
#
# Droid dreamzero baseline eval with guidance sweep.
#
#   (one-time)  python -m droid_baseline_eval.prepare_gt
#   (per run)   bash droid_baseline_eval/run_eval.sh 2000 4000 6000 8000 14000 22000
#
# By default sweeps GUIDANCES="0 1 3 7". Override via:
#   GUIDANCES="0 3" bash droid_baseline_eval/run_eval.sh 2000
#
...
EXPERIMENT="${EXPERIMENT:-ac_reason_embeddings_rectified_flow_2b_256_320_droid_dreamzero}"
CONFIG_FILE="${CONFIG_FILE:-cosmos_predict2/_src/predict2/action/configs/action_conditioned/config.py}"
CKPTS_ROOT="${CKPTS_ROOT:-$IMAGINAIRE_OUTPUT_ROOT/cosmos_predict2_action_conditioned/cosmos_predict_v2p5/new_droid_baseline2_dreamzero/checkpoints}"

EVAL_DIR="$PROJECT_DIR/droid_baseline_eval"
...
if [[ $# -eq 0 ]]; then
    ...
    echo "Example (representative ckpts): $0 2000 4000 6000 8000 14000 22000"
...
if [[ ! -d "$GT_DIR" ]] || [[ -z "$(ls -A "$GT_DIR" 2>/dev/null)" ]]; then
    ...
    echo "        Run: python -m droid_baseline_eval.prepare_gt" >&2
    exit 1
fi

"""
Evaluate one (experiment, action_loader, dataset, split, iter) combination.

Reads generated <ep>_chunk.mp4 files produced by inference and pairs each
with the shared GT composite for the same episode, then computes PSNR /
SSIM / LPIPS per frame for each view defined by the dataset spec (including
the full stacked composite) + FVD on the stacked composite.

For single-view datasets the only "view" is ``stacked``; for multi-view
composites you also get each sub-region (wrist/left/right, or left/right).

Outputs (under --out-dir):
    per_episode.csv     one row per episode, columns per view
    per_timestep.npz    arrays [N_ep, T_max], NaN-padded, keyed {metric}_{view}
    summary.json        means per view, FVD on stacked, iter / dataset / N
...
from eval.datasets import View, get_spec
from eval.metrics import LPIPSWrapper, compute_frame_metrics, compute_fvd
...
def main():
    ...
    ap.add_argument("--gt-dir", type=Path, required=True,
                    help="directory of <ep>.mp4 GT composites (shared cache).")
    ap.add_argument("--out-dir", type=Path, required=True)
    ap.add_argument("--iter", type=int, required=True, help="checkpoint iter (for logging).")
    ap.add_argument("--dataset", required=True, help="drives per-view crop boxes + save fps.")
    ...
    for gt_path in gt_paths:
        ...
        for view in views:
            box = view.box_px(hw)
            fm = compute_frame_metrics(_crop(gt, box), _crop(gen, box), lpips_fn, skip_first=1)
        ...
        if args.save_videos:
            ...
            mediapy.write_video(videos_dir / f"{ep}_side_by_side.mp4", side_by_side, fps=video_fps)

    if not per_episode_names:
        raise RuntimeError(f"No paired episodes found between {args.gen_dir} and {args.gt_dir}")

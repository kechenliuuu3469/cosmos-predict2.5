#!/bin/bash
#SBATCH --job-name=cosmos_oxe_lam_bridge
#SBATCH --nodes=1
...
mkdir -p slurm_outputs/cosmos_oxe_lam_bridge
...
# Bridge-only LAM training. Bridge LAM latents live at:
#   $OXE_LAM_ROOT/bridge/$OXE_LAM_SUBDIR/{train,val}/<episode>/latent_actions.npy
export OXE_BASE_PATH=/scratch/gpfs/AM43/users/kl0820/datasets/oxe_mp4
export OXE_LAM_ROOT=/scratch/gpfs/AM43/users/kl0820/datasets/oxe_mp4
export OXE_LAM_SUBDIR=last_latent_action_v2

PYTHONPATH=$(pwd) \
torchrun --nproc_per_node=2 --master_port=12347 \
    -m scripts.train \
    --config=cosmos_predict2/_src/predict2/action/configs/action_conditioned/config.py \
    -- experiment=ac_reason_embeddings_rectified_flow_2b_256_320_oxe_lam_bridge \

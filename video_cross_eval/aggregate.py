...
``results/<split>/<dataset>/iter_XXXXXXXX/summary.json`` for the
checkpoints you care about.

Outputs (default under --out-dir = results/<split>/_aggregate):
    all_checkpoints.csv              one row per (dataset, view, iter)
    plots/<dataset>/<view>/{psnr,ssim,lpips}.png
    plots/<dataset>/fvd.png          FVD (on stacked only) vs iter
...
def _load_dataset(ds_root: Path):
    """Return a list of records (one per iter) for a single dataset."""
    records = []
...
def main():
    ...
    ap.add_argument(
        "--results-root",
        type=Path,
        default=Path(__file__).resolve().parent / "results" / "val",
    ...
    for ds in datasets:
        ...
        all_recs[ds] = recs
        for r in recs:
            # "views" key added by new evaluate.py; fall back gracefully.
    ...
    for ds, recs in all_recs.items():
        ...
        _plot_metric_vs_iter(
            iters, fvds,
            f"{ds} — FVD (stacked) vs iter",
            "FVD (lower = better)",
            ds_plot / "fvd.png",
        )

        # Per-view metric-vs-iter + per-timestep
        view_names = set()
        for r in recs:
        ...
        for view in sorted(view_names):
            ...
            for metric, key, better in [
                ...
                vals = [
                    (r.get("views") or {}).get(view, {}).get(key, float("nan"))
                    for r in recs
                ]
                _plot_metric_vs_iter(
                    iters, vals,
                    f"{ds} [{view}] — {metric} vs iter",
                    f"{metric} ({better} = better)",
                    v_plot / f"{metric.lower()}.png",
                )
                _plot_per_timestep(
                    recs, metric.lower(), view,
